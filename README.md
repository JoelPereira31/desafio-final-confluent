...
Comandos utilizados para listagem de identidades e ACLs:
...
1. **Aceitação de campo novo**: Adicionado com valor padrão (*default*), validado com sucesso pelo Schema Registry.
2. **Rejeição de alteração incompatível**: Bloqueio na remoção de campos obrigatórios para manter a integridade do contrato.
...
1. **Observabilidade**: Monitoramento contínuo de latência e consumo de bytes via métricas de ecossistema.
2. **Segurança**: Mapeamento rigoroso de Service Accounts e ACLs restritivas por tópico.
3. **Custos**: Arquitetura planejada em cluster *Basic* para otimização de recursos em ambiente de desenvolvimento.
4. **Confiabilidade**: Garantia de entrega idempotente e tratamento de falhas no fluxo de mensageria.
